...
//Filter the forms based in location 
	$forms = $connection->query("SELECT * from clients");
	...
	while($row = $forms->fetch_array()){
		...
		for($i=0;$i<sizeof($form_loc);$i++){
			...
			if($dist < 100 && $dup !='a' ){
				echo '<tr><td>'.$formname[$i].'</td><td>'.$formdes[$i].'</td><td>'.$date[$i].'</td><td><button class="btn" name="'.$row['UserID'].'" id="'.$formname[$i].'" value="'. $formsID[$i].'" onclick="Redirect(this.value,this.id,this.name);">Open</button</td></tr>';
			}
